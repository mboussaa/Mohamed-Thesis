...
Then, we apply the optimizations to the input source code program and execute the optimized code within a new container instance (step 2). While running the optimized code, we collect runtime monitoring data (steps 3, 4, and 5). More details about these three steps are provided in Chapter \ref{chap:docker}.
Finally, NOTICE assigns a performance/resource usage metric to the current solution (step 6). 
