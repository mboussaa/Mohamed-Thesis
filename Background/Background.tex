...
Another concrete example of how software diversity is managed in industry production is JHipster\footnote{https://jhipster.github.io/}. JHipster is an application generator based on YO generator which provides tools to generate quickly modern web applications using Java stack on the server side (using Spring Boot) and a responsive Web front-end on the client side (with AngularJS and Bootstrap).
...
%The middle-end performs optimizations on a form other than the source code or machine code. This source code/machine code independence is intended to enable generic optimizations to be shared between versions of the compiler supporting different languages and target processors.
%Last step is to perform more analysis, transformations and optimizations that are for a particular computer. Generates the target-dependent assembly code, performing register allocation in process. Performs optimizations of the target code utilization of the hardware, like figuring out how to keep parallel execution units busy by filling delay slots.
...
%As shown in Figure 2.2 This front/middle/back-end approach makes it possible to combine front ends for different languages with back ends for different CPUs. Practical examples of this approach are the GNU Compiler Collection, LLVM,[15] and the Amsterdam Compiler Kit, which have multiple front-ends, shared analysis and multiple back-ends.

%Compilers can be classified depending on the platform on which their generated code executes. Typically, compilers intend to generate the output of a back end is machine code specialized for a particular processor and operating system.
...
%Meanwhile, software is facing challenges of a similar magnitude,
%with major changes in the way software is deployed, is sold, and interacts with hardware

%Software use to be a lot performance, about productivity, mamanging complexity, glueing lot of pieces together and give programmers the right abstraction for how things really work and what the data really is
%Hardware is exposing us to more low level 
%Software wants to be higher and higher level
%hard and software are pulling us in opposite directions
%hard is exposing us to more and more complxity  and software is higher level which is big problem for both communities
%we need a marriage between hardware and software

%In embeded devices we have seen a system on a chip on your phone. Nvidea and qualcome  are chipping single ISA heteregenous processors but the problem is that software is not ready for this hetergenouty 
%hardware offer us energy efficiency but expose a lot of complexity for software engineers and developers. Software arestguliing with hetergenouity. If we add hetergenousty can we deal with that
%What we have to do as software community to adress hetergenouity, we need some part of the work load or some ubiquity because hardware specialization for every single application is not a sustainable way of building chips. In order to improve and get that energy efficiency we have to havve common attributes of workload or somthing that works across across a big class of workloads in order that innovation in hardware can be exploided in the softare as well 
