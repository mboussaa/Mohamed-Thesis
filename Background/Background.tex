...
\item Inline code expander: This model reads code as input and the builds new code that uses the input code as a base but has sections of the code expanded based on designs in the original code. 
...
\item Mixed code generator: This model has The same processing flow as the Inline Code Expander, except that the input file is a real source file that can be compiled and run.
...
%Memory and performance overhead  (Peak memory overhead using /usr/bin/time Time is efficient for memory usage?? )
