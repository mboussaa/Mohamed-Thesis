...
\section{Mapping software to hardware}
...
\subsection{Overview of the generative programming development process}
...
Among the main advantages that GP is offering, is the automatic code generation, highlighted with red box in Figure \ref{fig:background_overview2}. The automatic code generation involves two principal aspects: 
\begin{enumerate}
	\item the use of code generators to cope with the software diversity and produce code for a range of software languages and platforms. GP provides an abstraction layer that hide the technical and implantation details. Instead, it relies on code generators to automate the code transformation process and to target different range of software platforms.
...
Both, compilers and code generators, are highly configurable. Compilers, in the one hand, becomes very user-friendly, letting the user to easily introduce optimizations and customize the machine code for the target hardware machine. On the other hand, code generators are less configurable than compilers but offer the possibility to customize the generated code for the target software platform (e.g., select the target programming language, platform settings, general-purpose optimizations, etc.)

Meanwhile, compilers are more used than code generators because they are widely used and properly tested compared to code generators. However, code generators are not always well tested and less used because users lack of confidence on their correct functioning. As a consequence, they need to be properly tested.
...
\subsection{Stakeholders and their roles for testing highly configurable code generators}
\begin{figure}[h]
	\center
...
\begin{itemize}
	...
	\end{table}
	This table shows how huge is the code generator base code. With more than 1.800.000 lines of code, it is very hard to test th whole system. 
	...
	In case of non-executable models, verifying that the code produced has a correct behavior as it is described in the model becomes  difficult since it is not executable.   
...
\section{Summary: Testing and optimization challenges}
