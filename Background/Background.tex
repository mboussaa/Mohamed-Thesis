...
\end{figure}
Software development involves different stakeholders that play different roles in validating and testing the software development chain described previously.
Figure 2.6 depicts a use case diagram describing these different concerns, actors and roles for testing generators.
Basically, we distinguish two stakeholders for code generators and compilers testing: experts and developers. As shown in the bottom of Figure 2.6, the experts are the creators/maintainers of generators that provide their expertise and the methodology associated to the generators resulting in efficient code generation. They contribute to the software development community by providing new optimizations and compiler versions updates on the compiler side. For code generators, they may use their knowledge to build new platform-specific code generators or enhance existing code generators. 

Developers however, represent the group of users that have no knowledge/expertise about the way code is generated. Thus, they are unable to edit or maintain the internal behavior of generators (e.g., the case of commercial and off-the-shell code generators). In this case, generators are used as a black box during software development by engineers to ease code production. Developers may configure the set of optimizations for the compiler or maintain the generated code in case of automatic source code generation.

The uses cases highlighted in red in Figure 2.6 constitute the main tasks that we are addressing in this thesis. Our main concern in our proposal is to evaluate the generated code. On the one hand, we would help code generator users to automatically generate code for different target software platforms in order to detect code generator inconsistencies. This task may involve both experts and users.
On the other hand, we would help compiler users to auto-tune compilers through the use of optimizations provided by compilers experts. Similarly, this concerns both actors.
...
%Relation 1 et 2
%les répitions, uses cases, etc
%talk about non-functional properties in background
%Functional Testing of CG  in background
%NON-functional workshop or journal
%organisation
%SOTA oranisation
%2 pages for the reader
