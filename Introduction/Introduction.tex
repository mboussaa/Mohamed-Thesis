...
On the other hand, code generators are less configurable than compilers. Code transformations are internally managed during code generation without giving too much the opportunity to the user to customize the generated code. As an example, code generator creators define the code generator behavior by specifying all model-to-model, model-to-text or text-to-text transformation rules in order to generate code for a specific target.
To configure and test code generators, developers generally use to follow the cycle \textit{edit, compile, and test}. First, they edit the templates, mapping and transformation rules that define the code generation process, then run the generator to create the output files. The output files are then compiled along with the custom code and the application is tested. At this point if they find a problem in the generated code, they alter the templates or input of the generator and re-generate. This cycle is repeated as long as new changes are applied. In return, third-party code generator users who would use existing code generators, need just to provide the input program to the generator and specify the target software platform to be able to generate source code. In this case, the quality of generated code depends only on the efficiency of selected code generator for the target platform.
