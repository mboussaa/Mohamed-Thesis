...
Automatic code generation involves code generators, needed to transform the high-level system specifications (e.g., textual or graphical modeling language) into conventional source code programs (e.g., General-purpose Languages GPLs such as Java, C++, etc), and compilers needed to transform the high-level source code programs, that were manually written or automatically generated, into low-level machine code (i.e., binaries, executables). 
...
Moreover, modern generators (e.g., C compilers) become highly configurable, offering
