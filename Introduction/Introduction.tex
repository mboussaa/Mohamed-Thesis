...
In return, code generator users (clients) can integrate new existing code generators ("off-the-shell") to their software project. To do so, there is not too much configurations to set up, they need just to provide the input program to the generator and specify the target software platform to be able to generate source code. In this case, the quality of generated code depends only on the efficiency of the selected code generator for the target platform.
