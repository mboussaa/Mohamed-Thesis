...
This example shows how paintful it could be for the compiler users to tune compilers (through optimization flags) in order to satisfy different non-functional properties such as execution time, compilation time, code size, etc.
...
In case of using an off-the-shell code generator during software development, engineers need just to write/provide the input program to the generator and specify the target software platform to be able to automatically generate source code. In this case, the quality of generated code depends only on the efficiency of the selected code generators for the target platforms. 
For example, this is applied in the "write once, run everywhere" concept where users can benefit from a family of code generators (e.g., cross-platform code generators\cite{fumero2015runtime}) to generate from the manually written (high-level) code different artifacts for diverse software platforms.
...
In fact, the quality of the generated software by either compilers or code generators is directly correlated to the quality of the code generator. As the quality of the code generator is maintained and improved, the quality of generated software artifacts also improves.
