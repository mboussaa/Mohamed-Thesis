...
We conduct a statistical analysis of the non-functional outputs of original test inputs in order to find the right threshold value $T$ and then, detect inconsistencies.
To quantify this variation we propose two methods: Principal components analysis (PCA) and R charts (R-chart).
In the following, we describe these two statistical methods:
\paragraph{PCA}~\\ 
\paragraph{R-chart}~\\
%\subparagraph{title} 
