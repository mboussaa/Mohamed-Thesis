...
The central line (in green) corresponds to $R$-$bar$. This value changes from one benchmark to another depending on the average of $R$ across all test suites in the benchmark. As a consequence, $LCL$, which is a function of $R$-$bar$ changes as well if we add new test suites to the experiments. We recall that we have classified the R-chart approach as input sensitive since the average variation $R$-$bar$ and the threshold value $LCL$ change dynamically by adding new follow-up test suites to the corresponding benchmark. 
...
We apply the PCA approach as an alternative to the R-chart approach. So,
\begin{figure}
	\centering  
